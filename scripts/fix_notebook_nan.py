...
# Handle NaN values in cleaned_comments (e.g. comments that became empty after cleaning)
...
for target in targets:
    ...
    # Filter out NaN if any in target (though VADER/TextBlob shouldn't produce NaNs usually)
    valid_idx = y.notna()
...
for cell in nb['cells']:
    if cell['cell_type'] == 'code' and ('# ML-Based Sentiment Analysis' in cell['source'] or 'TfidfVectorizer' in cell['source']):
        # We look for the cell that does the ML analysis. 
        # The previous update script put '# ML-Based Sentiment Analysis' at the top.
        if 'targets =' in cell['source'] or 'LogisticRegression' in cell['source']:
             cell['source'] = new_ml_code
             found = True
             break

if not found:
    print("ML cell not found to update. Appending...")
